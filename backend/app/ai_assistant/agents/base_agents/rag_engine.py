...
class RagEngine(BaseAgent):
    ...
    def _build_citation(self, doc, paper_number: int) -> str:
        ...
        return f'[Doc {paper_number}: "{article_title}", pág. {page}]'
    ...
    def retrieve_for_article_ids(
        ...
        """
        Recuperación RAG dirigida por el grafo: busca chunks del vector store
        filtrando por los article_ids indicados (expansión del grafo).

        Estrategia:
        1. Recupera ``fetch_k`` candidatos del vector store por similitud.
        2. Filtra manualmente por ``article_id`` en los metadatos.
        3. Limita a ``k_per_article`` chunks por artículo.
        4. Segundo pase para artículos sin cobertura usando el article_id como query.

        Parameters
        ----------
        user_message      : Query original del usuario.
        article_ids       : IDs de artículos identificados por el grafo.
        k_per_article     : Máximo de chunks por artículo.
        fetch_k           : Candidatos iniciales del vector store.
        max_context_chars : Límite de caracteres del bloque devuelto.

        Returns
        -------
        Bloque ``--- CONTEXTO DIRIGIDO POR GRAFO ---`` o cadena vacía.
        """
        if self.vector_store is None or not article_ids:
            return ""

        target_set = {str(a) for a in article_ids}
        counts: Dict[str, int] = {a: 0 for a in target_set}
        selected: List = []

        # Primer pase: candidatos por similitud con la query
        try:
            candidates = self.vector_store.similarity_search(user_message, k=fetch_k)
        except Exception:
            candidates = []

        for doc in candidates:
            aid = str((doc.metadata or {}).get("article_id") or "")
            if aid in target_set and counts[aid] < k_per_article:
                selected.append(doc)
                counts[aid] += 1

        # Segundo pase: artículos con cobertura cero
        for aid in [a for a, cnt in counts.items() if cnt == 0]:
            try:
                extra = self.vector_store.similarity_search(aid, k=k_per_article)
            except Exception:
                continue
            for doc in extra:
                doc_aid = str((doc.metadata or {}).get("article_id") or "")
                if doc_aid == aid and counts[aid] < k_per_article:
                    selected.append(doc)
                    counts[aid] += 1
        ...
        retrieved = "\n\n".join(context_blocks)
        return (
            "\n--- CONTEXTO DIRIGIDO POR GRAFO ---\n"
            "(Fragmentos de artículos identificados como relacionados por el grafo)\n"
            f"{retrieved}\n"
            "--- FIN DEL CONTEXTO DIRIGIDO ---\n\n"
        )
